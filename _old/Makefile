...
binaries: base dev
	rm -rf tmp/
	mkdir -p tmp/
	docker run -ti --rm -v $(GOPATH)/src:/go/src rubenv/osmtopo-dev bash -c "go get -tags=embed -v -t -d ./... && go install -x -tags=embed -v github.com/rubenv/osmtopo/osmtopo && go build -tags=embed -v -o tmp/osmtopo ./bin/osmtopo && go test -tags=embed -bench=. -benchmem -cover github.com/rubenv/osmtopo/..."
